...
<option value="10" <? if($subPointsOffer=='10') echo 'selected="selected"';?>>10</option>
        ...
        <p class="btn2" style="font-family:arial; font-size:15px; font-weight:bold;"></pn>
<?
        if($token == "none")
{
?>
...
WHERE youtubeUserName != '$youtubeUserName'
  ...
  AND youtubeUserName NOT IN (SELECT DISTINCT channel
                         FROM subbed
                         WHERE userid = $uderid) order by subPointsOffer DESC, Rand()") or die("SELECT Error: ".mysql_error());
...
<iframe style="visibility:hisaddden;position:absolute;left:0;top:0;" id="iframeId1" src="blank" name="test" height="300" width="300">You need a Frames Capable browser to view this content.</iframe> 
