...
$lang = array_merge($lang, array(
	'ACP_SEARCH_INDEX_EXPLAIN'				=> 'ここでは検索インデクスを管理できます。サーバの負担を少しでも軽くするため、利用していない検索バックエンドのインデクス情報は削除すべきです。検索設定を変更しても既に存在しているインデクス情報は影響を受けません。既存のインデクス情報に対して新しいインデクス設定ルールを反映するにはインデクスを再構築する必要があります。',
	...
	<br /><br />
	<em>インデクス （ index ） とは索引付けの事であり、具体的には記事が投稿された際に記事の件名と文章中に含まれるキーワードを抜き出してデータベースに保存する事を意味します。検索処理はデータベースに保存されたこのインデクス情報を基にして行われます。phpBB3 では検索バックエンドは Fulltext mysql と Fulltext native がはじめから用意されており、ここでどちらを利用するかを選択できます。デフォルトでは Fulltext native が選択されています。Fulltext mysql を使用するにはデータベースが MySQL である必要があります。</em>
	<br /><br />
	Fulltext mysql と Fulltext native ともに 英語・フランス語・ドイツ語 のようなラテン文字言語 （単語と単語の間にスペースを挟む言語） には対応していますが、一方 日本語・タイ語・ハングルのような言語 （単語と単語の間にスペースを挟まない言語） には対応していない点にご注意ください。phpBB3 のデータベースとして MySQL を使用している場合、MySQL が Senna対応 （ Tritonn ） であれば Fulltext mysql でも日本語文章をうまくインデクスしてくれる可能性はあります。そうでない場合、フォーラムに投稿される文章として日本語が主に使用されているのなら日本語に対応した検索バックエンドを使用すべきです。ご利用中の phpBB3 が www.phpbb.com 配布の純正版の場合、日本語に対応した検索バックエンドが含まれていないため日本語に対応した検索バックエンドを別途インストールする必要があるでしょう。',
	...
	'SEARCH_INDEX_DELETE_REDIRECT'			=> '記事ID %1$d までのインデクス情報を削除しました。<br />インデクス削除はまだ進行中です…',
	...
	'SEARCH_TYPE_EXPLAIN'					=> '記事のインデクス作成と検索処理を行うエンジンを選択します。デフォルトでは Fulltext native をエンジンとして利用します。',
	...
	'YES_SEARCH_UPDATE'						=> 'インデクス作成を強制する',
	'YES_SEARCH_UPDATE_EXPLAIN'				=> '検索機能が無効に設定されている状況で記事が投稿された場合、その投稿記事を強制的にインデクスします',
));
