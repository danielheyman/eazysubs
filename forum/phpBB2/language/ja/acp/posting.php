...
$lang = array_merge($lang, array(
	...
	'tokens'	=>	array(
		...
		'INTTEXT'		=> 'ラテン文字、半角数字(0-9)、半角スペース、コンマ(,)、マイナス(-)、プラス(+)、ハイフン(-)、アンダーライン(_)、ドット(.)、半角スペース( )',
		...
		'LOCAL_URL'		=> 'ローカルURL。トピック表示ページから見た相対的な URL である必要があります。サーバ名（ホストまたはドメイン）とプロトコルを含んではいけません。',
...
$lang = array_merge($lang, array(
	'ACP_ICONS_EXPLAIN'		=> 'ここではトピックアイコンの追加、削除、編集を行うことができます。これらのアイコンはフォーラム閲覧ページでトピックまたは返信記事の件名に並べて表示されます。アイコンパックのインストールと作成を行う事もできます。',
	'ACP_SMILIES_EXPLAIN'	=> 'スマイリー （別称: エモーションアイコン） は感情や気持ちを表現するために用いられる小さな画像です。ここではユーザーが記事とプライベートメッセージで使用できるスマイリーの作成、削除、編集を行うことができます。スマイリーパックのインストールと作成を行うこともできます。',
...
// Disallow Usernames
$lang = array_merge($lang, array(
	'ACP_DISALLOW_EXPLAIN'	=> 'ここでは禁止ユーザー名の追加と解除を行うことができます。禁止ユーザー名をユーザー登録の際に使用する事はできません。部分一致としてワイルドカード(*)を使用できます。すでに存在しているユーザー名を禁止ユーザー名リストに加える事はできない点にご注意ください。',
	...
	'DISALLOWED_ALREADY'		=> '入力したユーザー名は既に禁止ユーザー名リストに存在しているか、言語フィルタリストに存在しているか、ユーザー登録されています',
