...
<?php $_memberrow_count = (isset($this->_tpldata['memberrow'])) ? sizeof($this->_tpldata['memberrow']) : 0;if ($_memberrow_count) {for ($_memberrow_i = 0; $_memberrow_i < $_memberrow_count; ++$_memberrow_i){$_memberrow_val = &$this->_tpldata['memberrow'][$_memberrow_i]; if ($this->_rootref['S_SHOW_GROUP']) {  if ($_memberrow_val['S_FIRST_ROW'] && $_memberrow_val['S_GROUP_LEADER']) {  ?>
	...
	<tr class="efekt">
		...
		<td class="row1" align="center">&nbsp;<?php echo $_memberrow_val['ROW_NUMBER']; ?>&nbsp;</td>
		<td class="row2" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>"><?php echo $_memberrow_val['USERNAME_FULL']; if ($this->_rootref['S_SELECT_SINGLE']) {  ?> [&nbsp;<a href="#" onclick="insert_single('<?php echo $_memberrow_val['A_USERNAME']; ?>'); return false;"><?php echo ((isset($this->_rootref['L_SELECT'])) ? $this->_rootref['L_SELECT'] : ((isset($user->lang['SELECT'])) ? $user->lang['SELECT'] : '{ SELECT }')); ?></a>&nbsp;]<?php } ?></td>
		<td class="row1" align="center" nowrap="nowrap">&nbsp;<?php echo $_memberrow_val['JOINED']; ?>&nbsp;</td>
		<td class="row2" align="center"><?php echo $_memberrow_val['POSTS']; ?></td>
		<td class="row1" align="center"><?php if ($_memberrow_val['RANK_IMG']) {  echo $_memberrow_val['RANK_IMG']; } else { echo $_memberrow_val['RANK_TITLE']; } ?></td>
		<td class="row2" align="center">&nbsp;<?php if ($_memberrow_val['U_PM']) {  ?><a href="<?php echo $_memberrow_val['U_PM']; ?>"><?php echo (isset($this->_rootref['PM_IMG'])) ? $this->_rootref['PM_IMG'] : ''; ?></a><?php } ?>&nbsp;</td>
		<td class="row1" align="center">&nbsp;<?php if ($_memberrow_val['U_EMAIL']) {  ?><a href="<?php echo $_memberrow_val['U_EMAIL']; ?>"><?php echo (isset($this->_rootref['EMAIL_IMG'])) ? $this->_rootref['EMAIL_IMG'] : ''; ?></a><?php } ?>&nbsp;</td>
		<td class="row2" align="center">&nbsp;<?php if ($_memberrow_val['U_WWW']) {  ?><a href="<?php echo $_memberrow_val['U_WWW']; ?>"><?php echo (isset($this->_rootref['WWW_IMG'])) ? $this->_rootref['WWW_IMG'] : ''; ?></a><?php } ?>&nbsp;</td>
