...
$lang = array_merge($lang, array(
	...
	'DB_HOST_EXPLAIN'			=> 'DSN とはデータソース名のことです。 ODBC 経由でデータベース接続を行っている場合は DSN を入力する必要があります。 PostgreSQL の場合、UNIXドメインソケット 経由でローカルホストに接続するには localhost を、 TCP 経由で接続するには 127.0.0.1 を入力してください。',
	'DB_NAME'					=> 'データベースの名前',
	...
	'INST_ERR_FATAL_DB'			=> '致命的かつ復元不可能なデータベースエラーが発生しました。指定されたユーザーに <code>CREATE TABLES</code> または <code>INSERT</code> などの SQL 命令の実行権限が与えられていない事が原因と考えられます。詳細は下記をご覧ください。まず最初にサーバ管理者にこの事をお問い合わせください。それでも解決しない場合、phpBBサポートフォーラム をご利用ください。',
	...
	'MBSTRING_CHECK_EXPLAIN'				=> '<strong>必須</strong> - <samp>mbstring</samp> はマルチバイト文字を扱うための PHP 拡張モジュールです。mbstring 設定オプションのいくつかは phpBB3 と相性が良くないため、それらのオプションを無効にしておく必要があります。もし表示が “いいえ” の場合、phpBB3 ルートディレクトリ直下のファイル .htaccess に下記コードを追加することで解決することがあります。<br />
<var>
--- コード ---<br />
...
-------------
	...
	'PHP_REGISTER_GLOBALS_EXPLAIN'	=> 'この設定オプションが有効に設定されていても phpBB3 は動作するでしょう。しかしセキュリティ上の理由で、もし可能であるなら register_globals を無効にしておくことを強く勧めます。もし表示が “いいえ” の場合、phpBB3 ルートディレクトリ直下のファイル .htaccess に下記コードを追加することで解決することがあります。<br />
<var>
--- コード ---<br />
...
$lang = array_merge($lang, array(
	...
	'SOME_QUERIES_FAILED'		=> 'クエリの実行に失敗しました。実行クエリとそのエラーが下に表示されています。',
	'SQL'						=> 'SQL',
